...
protected:
    ...
    void RunThrust()
    {
        ...
        thrust_result                     = 
            thrust::reduce(dev_vect.begin(), dev_vect.end(), (T)0.0, thrust::plus<T>());
    }

    void RunHIP()
    {
        thrust::device_vector<T> dev_vect = input;
        gpu_result                        = reduce_gpu_one_block(
            dev_vect, (T)0.0, thrust::plus<T>(), reduce_config.threads_per_block);
    }
